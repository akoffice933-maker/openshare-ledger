...
|---|---|---|
| Реальная отправка якоря | Нет ключей и бюджета | Служба метки времени RFC 3161 или прозрачный лог наподобие Sigstore Rekor; интерфейс уже готов (`--publish`) |
...
2. **Реальный якорь** — служба метки времени RFC 3161 (раз и навсегда) или прозрачный лог наподобие Sigstore Rekor (ежедневный батч-хеш).
...
Это не формальность, а то, что удерживает всю конструкцию вне режима регулирования ценных бумаг.
